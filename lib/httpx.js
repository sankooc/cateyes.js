...
//                delete reqList[id+source.index];
                deferred.resolve();
...
function download(video){
    ...
    if(sources.length == 1){
        tasks.push(function(cb){
            ...
            _download(sources[0],video._id).then(function(ret){
                cb(null,ret);
            },function(err){
                cb(err);
            })
        ...
        for(var i= 0;i<sources.length;i++){
            ...
            var func = (function(source,title){
                return function(cb){
                    ...
                    source.title = title
                    _download(source,video._id).then(function(){
                        cb(null);
                    },function(err){
                        cb(err);
                    })
