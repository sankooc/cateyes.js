        ...
        for(var i= 0;i<sources.length;i++){
            ...
            var func = (function(source,title){
                return function(cb){
                    ...
                    source.title = title
                    _download(source).then(function(ret){
                        cb(null,ret);
                    },function(err){
                        cb(err);
                    })
...
var metadata = youku.parseMetadata(page1);
metadata.then(function(ret){
    console.log(ret);
    var report ={
        'metadata':ret,
        'parameter':{'type':'flv'}};
    youku.getResource(report).then(function(video){
        setInterval(function(){
            console.log('***********');
            for(var i =0; i<video.getSource().length;i++){
                var source = video.getSource()[i];
                if(source.current < source.total){
                    console.log('inx:%s current:%s rest:%s',source.index,source.current,source.total-source.current);
                }else{
//                    console.log('inx:%s current:%s',source.index);
                }
//                console.log(video.getSource()[i].current);
            }
            console.log('***********');
        },2000);

        download(video).then(function(){
