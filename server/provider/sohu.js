...
function _parse(context,url,callback){
    waterfall([
        ...
        ,function(text,callback){
            ...
            if(!context.data)
                return callback('parse vid failed:'+url + ":context:"+JSON.stringify(context))
            callback()
        }
    ],callback)
}
            ...
            waterfall([
                ...
                ,function(window,callback){
                    ...
                    for(var i = 0 ;i<scripts.length;i++){
                        var cont = scripts.eq(i).text();
                        if(/var\s+vid/.test(cont)){
//                            console.log(cont)
                            eval(cont)
                            break;
                        }
                    }
                    if(!vid) callback('no vid')
//                    if(type_reg.test(context.url)){
...
function parse(context,callback){
    context.vType = context.vType || context.metadata.vType
    if(!context.vType) return callback('no type')
    var infoUrl
